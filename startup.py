...
from utils.hw_api import (
  ...
  api_batchmesh2, api_tetmesh, exec_tcl, get_model_name,
...
def assign_all():
  """批量赋属性: assign_all()
  根据缓存的 classify 结果: 薄壁件→PSHELL, 厚实体→PSOLID
  """
  ...
  thick_ids = [int(p.comp_id) for p in _classify_result.thick_solid_parts]
  pa.run(thin_ids, thick_ids)
  print(f"属性完成: {len(thin_ids)} 个 PSHELL + {len(thick_ids)} 个 PSOLID")
...
def classify(show_details=False):
  ...
  if result.success:
    ...
    print(f"  厚实体(体网格):   {len(result.thick_solid_parts)} 个")
    print(f"{'='*60}")
    if show_details:
      for label, parts in [
        ("薄壁件_壳网格", result.thin_parts),
        ("微小件_可忽略", result.small_parts),
        ("实体件_体网格", result.thick_solid_parts),
      ]:
      ...
      print()
    print(f"  下一步: del_small() 删除微小件 → 确认分类正确 → batchmesh()(薄壁) + solid_mesh()(厚实体)")
    print(f"  手动纠正: move_to_thick(*ids) 或 move_to_thin(*ids)")
  return c

def move_to_thick(*comp_ids):
  """手动纠正: 把指定 comp_id 从薄壁件移到厚实体: move_to_thick(1, 2, 3)"""
  global _classify_result
  if _classify_result is None: print("请先运行 classify()"); return
  moved = []
  for cid in comp_ids:
    cid = int(cid)
    for lst in [_classify_result.thin_parts, _classify_result.small_parts]:
      for p in list(lst):
        if p.comp_id == cid:
          lst.remove(p)
          p.category = "thick_solid"
          _classify_result.thick_solid_parts.append(p)
          moved.append(cid)
          break
  if moved:
    # 重新输出统计
    ...
    print(f"  厚实体: {len(_classify_result.thick_solid_parts)} → solid_mesh()")
  else:
    print("未找到匹配的薄壁件")

def move_to_thin(*comp_ids):
  """手动纠正: 把指定 comp_id 从厚实体移到薄壁件: move_to_thin(1, 2, 3)"""
  global _classify_result
  if _classify_result is None: print("请先运行 classify()"); return
  moved = []
  for cid in comp_ids:
    cid = int(cid)
    for p in list(_classify_result.thick_solid_parts):
      if p.comp_id == cid:
        _classify_result.thick_solid_parts.remove(p)
        p.category = "thin"
...
def del_small():
  ...
  if _classify_result:
    ...
    c._results = {p.comp_id: p for p in
      _classify_result.thin_parts + _classify_result.small_parts + _classify_result.thick_solid_parts}
...
def solid_mesh(size=8.0):
  """厚实体体网格: solid_mesh(size=8.0)
  只对厚实体运行 batchmesh2（solid 模式: 四面体网格）
  """
  global _classify_result
  if _classify_result is None: print("请先运行 classify()"); return
  pids = [int(p.comp_id) for p in _classify_result.thick_solid_parts]
  if not pids: print("没有厚实体"); return
  print(f"四面体网格: {len(pids)} 个厚实体 → tetmesh")
  api_tetmesh(pids, elem_size=size)
  print(f"体网格完成: size={size}mm")
